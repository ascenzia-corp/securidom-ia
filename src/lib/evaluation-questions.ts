...
export const QUESTIONS: Question[] = [
  // ── Module 1 : État des lieux de l'IA ──────────────────────────────
  {
    id: 0,
    module: "m1",
    ...
    correct: 1,
    explanation:
      "L'IA prédictive détecte des patterns dans les données existantes pour faire des prédictions (ex: maintenance prédictive). L'IA générative crée du contenu nouveau : texte, images, code (ex: ChatGPT, Claude).",
  },
  {
    id: 1,
    module: "m1",
    ...
    correct: 1,
    explanation:
      "Les LLM peuvent générer des informations fausses avec un ton très convaincant. C'est pourquoi la vérification humaine reste indispensable.",
  },
  {
    id: 2,
    module: "m1",
    ...
    correct: 1,
    explanation:
      "Dans le secteur de la sécurité, les données clients (plans de sûreté, rapports d'intervention) sont confidentielles. L'hébergement européen et les outils respectant le RGPD sont essentiels.",
  },
  {
    id: 3,
    module: "m1",
    ...
    correct: 1,
    explanation:
      "Un agent IA est un système qui reçoit un objectif, planifie les étapes, utilise des outils et livre un résultat — de façon autonome. C'est l'évolution naturelle des chatbots.",
  },
  {
    id: 4,
    module: "m1",
    ...
    correct: 1,
    explanation:
      "L'IA est un outil d'aide à la décision, pas un remplaçant du jugement humain. Sur les décisions critiques (sécurité des personnes, conformité légale), l'humain reste indispensable.",
  },

  // ── Module 2 : Maîtriser les LLM ──────────────────────────────────
  {
    id: 5,
    module: "m2",
    ...
    correct: 1,
    explanation:
      "Règle d'or : ne JAMAIS saisir de données personnelles, confidentielles ou stratégiques dans un LLM grand public. Les données peuvent être utilisées pour l'entraînement des modèles.",
  },
  {
    id: 6,
    module: "m2",
    ...
    correct: 1,
    explanation:
      "Un Projet Claude permet de donner à l'IA un contexte permanent (procédures, données de référence) via des instructions système et des fichiers. L'IA « connaît » alors votre métier.",
  },
  {
    id: 7,
    module: "m2",
    ...
    correct: 1,
    explanation:
      "Le few-shot consiste à montrer 2-3 exemples de ce que vous attendez. L'IA comprend alors le format, le ton et le niveau de détail souhaités.",
  },
  {
    id: 8,
    module: "m2",
    ...
    correct: 1,
    explanation:
      "L'itération est la clé. Dites à l'IA ce qui manque : « C'est trop long, concentre-toi sur les 3 points clés » ou « Reformule avec un ton plus professionnel ».",
  },
  {
    id: 9,
    module: "m2",
    ...
    correct: 2,
    explanation:
      "Mistral AI est une entreprise française fondée en 2023. C'est le seul LLM souverain européen parmi les leaders du marché. Important pour les enjeux de souveraineté des données.",
  },

  // ── Module 3 : Identifier ses cas d'usage ──────────────────────────
  {
    id: 10,
    module: "m3",
    ...
    correct: 1,
    explanation:
      "D = Douleur. C'est l'intensité du problème ressenti au quotidien. Plus la douleur est forte (note élevée), plus le cas d'usage a de potentiel.",
  },
  {
    id: 11,
    module: "m3",
    ...
    correct: 2,
    explanation:
      "Score > 200 avec un effort faible = Quick Win. Ce cas d'usage est fréquent (O=5), douloureux (D=4), impactant (I=4) et techniquement faisable (T=5). C'est un candidat idéal pour une mise en œuvre rapide.",
  },
  {
    id: 12,
    module: "m3",
    ...
    correct: 1,
    explanation:
      "La matrice effort/impact est l'outil clé. Quick Win = fort impact, faible effort. Projet stratégique = fort impact, effort élevé (nécessite planification).",
  },

  // ── Module 4 : Feuille de route ────────────────────────────────────
  {
    id: 13,
    module: "m4",
    ...
    correct: 1,
    explanation:
      "Les 30 premiers jours servent à créer un « wow moment » rapide. Déployez Claude Projects, automatisez un rapport, testez sur un cas réel. Prouvez la valeur avant de structurer.",
  },
  {
    id: 14,
    module: "m4",
    ...
    correct: 1,
    explanation:
      "Le piège n°1 est de vouloir tout faire en même temps. Commencez par 2-3 Quick Wins, prouvez la valeur, puis élargissez. « Process First, Technology Second. »",
  },
];

/**
 * Returns a feedback message based on the total score (out of 15).
 */
export function getResultMessage(score: number): string {
  if (score >= 13) {
    return "Excellent ! Vous maîtrisez les fondamentaux de l'IA appliquée à votre métier. Vous êtes prêt(e) à passer à l'action.";
  }
  if (score >= 10) {
    return "Très bien ! Vous avez une bonne compréhension des concepts clés. Quelques points à consolider pour être 100 % opérationnel(le).";
  }
  if (score >= 7) {
    return "Bien ! Les bases sont là, mais certains modules méritent d'être revus pour renforcer votre maîtrise.";
  }
  return "Des progrès à faire. Nous vous recommandons de revoir les modules où vous avez eu le plus de difficultés.";
}

/**
 * Given an array of 15 answers (0 = wrong, 1 = correct for each question index),
 * returns the number of correct answers and total questions per module.
 */
export function computeModuleScores(
  ...
  for (const mod of MODULES) {
    let correct = 0;
    for (const qIndex of mod.questions) {
      if (answers[qIndex] === 1) {
        correct++;
      }
    }
    scores[mod.id] = { correct, total: mod.questions.length };
  }
